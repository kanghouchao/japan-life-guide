...
プロジェクトを開いたら、効率的な開発のためにVS Codeの機能を活用しましょう：

##### **エクスプローラーパネルの活用**

1. **左サイドバーのエクスプローラー**（📁アイコン）でファイル構造を確認
2. **よく使うファイル**：
   - `src/index.html` - ホームページ
   - `src/pages/` - 各コンテンツページ
   - `src/css/main.css` - 共通スタイル
   - `src/js/main.js` - 多言語機能

##### **ライブプレビューの使用**

1. **Live Server拡張機能**が有効になっていることを確認
2. HTMLファイルを右クリック→「**Open with Live Server**」
3. ブラウザで `http://127.0.0.1:5500` が自動的に開きます
4. ファイルを保存すると、ブラウザが**自動的にリロード**されます
...
**シナリオ**: 「交通情報ページ」を追加する場合

##### ステップ1: HTMLファイルの作成

1. `src/pages/` フォルダを右クリック
2. 「新しいファイル」を選択
3. `transportation.html` として保存
4. 既存ページ（例：`registration.html`）をコピーして基本構造を作成

##### ステップ2: ナビゲーションの更新

各HTMLファイル（`index.html` および `src/pages/` 内の全ファイル）のナビゲーション部分に新しいリンクを追加：

```html
<li><a href="./transportation.html" data-i18n="nav.transportation">交通情報</a></li>
```

##### ステップ3: 翻訳の追加

`src/js/translations.json` に各言語の翻訳を追加：

```json
{
  "ja": {
    "nav.transportation": "交通情報",
    "transportation.title": "交通情報",
    "transportation.description": "日本の交通システムについて説明します。"
  },
  "zh": {
    "nav.transportation": "交通信息",
    "transportation.title": "交通信息", 
    "transportation.description": "说明日本的交通系统。"
  },
  // 他の言語も同様に追加
}
```
